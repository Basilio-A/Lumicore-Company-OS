...
import { Plus, FileText, Trash2, TrendingUp, DollarSign, PieChart, Upload, Tag } from 'lucide-react';
...
const ROLE_COLORS_INV: Record<string, string> = { founder: '#6C63FF', investor: '#10B981', shareholder: '#F59E0B', employee: '#3B82F6' };
...
export default function InvestorPortalPage() {
  ...
  // profileRoleMap: holder_name -> role, for coloring
  const [profileRoleMap, setProfileRoleMap] = useState<Record<string, string>>({});
  ...
  const load = async () => {
    ...
    setDocuments(docs || []);
    const { data: profs } = await supabase.from('profiles').select('full_name, role').eq('status', 'active');
    const rm: Record<string, string> = {};
    for (const p of profs || []) rm[p.full_name] = p.role;
    setProfileRoleMap(rm);
  };
  ...
  // total equity always auto-calculates — never manually overridden
  const totalEquity = totalShares * sharePrice;

  const holderColor = (holderName: string, shareClass: string) => {
    const role = profileRoleMap[holderName];
    if (role && ROLE_COLORS_INV[role]) return ROLE_COLORS_INV[role];
    return CLASS_COLORS[shareClass] || '#9CA3AF';
  };
        ...
        <>
          <div className="grid lg:grid-cols-3 gap-4 mb-6">
            ...
            <Card className="p-5">
              ...
              <div className="text-3xl font-display font-bold text-[var(--text)]">{formatCurrency(convert(totalEquity), currency)}</div>
              <div className="text-xs text-muted mt-1">{holdings.length} holders · {totalShares.toLocaleString()} shares · {formatCurrency(convert(sharePrice), currency)}/share</div>
              <div className="text-xs text-muted mt-0.5">Edit share price in the Equity page</div>
            </Card>
          </div>

          <div className="grid lg:grid-cols-2 gap-6 mb-6">
            <Card className="p-5">
              <h3 className="font-semibold text-[var(--text)] mb-4 flex items-center gap-2"><TrendingUp className="w-4 h-4 accent" /> Cap Table Summary</h3>
              {holdings.length === 0 ? <p className="text-sm text-muted py-4">No equity data available.</p> : (
                <div className="space-y-2">
                  {holdings.map((h) => {
                    ...
                    const color = holderColor(h.holder_name, h.share_class);
                    return (
                      <div key={h.id} className="flex items-center gap-3">
                        <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: color }} />
                        <span className="text-sm text-[var(--text)] flex-1 truncate">{h.holder_name}</span>
                        <div className="w-24 h-1.5 rounded-full surface-2 overflow-hidden"><div className="h-full" style={{ width: `${pct}%`, backgroundColor: color }} /></div>
                        <span className="text-sm font-semibold text-[var(--text)] tabular-nums w-16 text-right">{pct.toFixed(1)}%</span>
                      </div>
                    );
                  })}
                </div>
...
function MemoEditor({ memo, onClose, onSaved }: { memo: InvestorMemo; onClose: () => void; onSaved: () => void }) {
  ...
  const [tags, setTags] = useState((memo as any).tags || []);
  const [tagInput, setTagInput] = useState('');
  const [saving, setSaving] = useState(false);
  const save = async () => {
    setSaving(true);
    await supabase.from('investor_memos').update({ title: title.trim(), content, tags }).eq('id', memo.id);
    setSaving(false);
    onSaved();
  };
  return (
...
function MemoCreator({ currentUserId, onClose, onCreated }: { currentUserId: string | null; onClose: () => void; onCreated: () => void }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    await supabase.from('investor_memos').insert({ title: title.trim(), content, tags, created_by: currentUserId });
    setSaving(false);
    onCreated();
    onClose();
  };
...
function TagInput({ tags, setTags, tagInput, setTagInput }: { tags: string[]; setTags: (t: string[]) => void; tagInput: string; setTagInput: (s: string) => void }) {
  const addTag = () => {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-2">
        {tags.map((t) => (
          <span key={t} className="inline-flex items-center gap-1 rounded-md surface-2 px-2 py-0.5 text-xs text-[var(--text)]">
            <Tag className="w-2.5 h-2.5" /> {t}
            <button onClick={() => setTags(tags.filter((x) => x !== t))} className="text-muted hover:text-rose-500">×</button>
          </span>
        ))}
      </div>
