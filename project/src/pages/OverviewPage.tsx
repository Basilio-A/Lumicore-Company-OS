...
export default function OverviewPage() {
  ...
  if (isInvestor) { navigate('/company/investors'); return null; }
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap gap-2">
        {PRESET_COLORS.map((c) => (
          <button
            ...
            type="button"
            onClick={() => onChange(c)}
            className={`w-8 h-8 rounded-lg transition-transform hover:scale-105 ${color === c ? 'ring-2 ring-offset-2 ring-offset-[var(--surface)] ring-[var(--text)] scale-110' : ''}`}
            style={{ backgroundColor: c }}
            title={c}
          />
        ))}
        {/* Custom color pin-drop / native picker */}
        <label
          className="w-8 h-8 rounded-lg border-2 border-dashed border-app flex items-center justify-center cursor-pointer hover:border-[var(--accent)] transition-colors relative overflow-hidden"
          title="Custom color"
        >
      ...
      <div className="flex items-center gap-2">
        ...
        <input
          ...
          className="text-xs rounded-lg surface-2 border border-app px-2 py-1 w-24 text-[var(--text)] focus:outline-none focus-visible:ring-1 focus-visible:ring-[var(--accent)] font-mono"
          placeholder="#6C63FF"
          maxLength={7}
        />
        <span className="text-xs text-muted">Hex or pick a color above</span>
      </div>
    </div>
  );
}
  ...
  return (
    <Modal open onClose={onClose} title={product ? 'Edit Product' : 'New Product'} className="max-w-lg">
      <div className="p-5 space-y-4">
        ...
        <div><label className="block text-xs font-medium text-muted mb-1.5">Logo URL</label><Input value={logoUrl} onChange={(e) => setLogoUrl(e.target.value)} placeholder="https://… (leave blank for letter avatar)" /></div>
        ...
        <div><label className="block text-xs font-medium text-muted mb-1.5">Color theme</label><ColorPicker color={color} onChange={setColor} /></div>
